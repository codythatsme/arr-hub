...
describe("ProfileService", () => {
  ...
  it.effect("create with quality items persists them", () =>
    Effect.gen(function* () {
      ...
      // Group children
      const children = result.qualityItems.filter(
        (i) => i.groupName === "WEB 1080p" && i.qualityName !== null,
      )
  ...
  it.effect("update replaces quality items", () =>
    Effect.gen(function* () {
      ...
      const created = yield* svc.create({
        name: "Items",
        qualityItems: [{ qualityName: "WEBDL-1080p", groupName: null, weight: 1, allowed: true }],
      })
