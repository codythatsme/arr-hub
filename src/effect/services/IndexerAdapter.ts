...
export interface IndexerAdapter {
  ...
  readonly search: (query: SearchQuery) => Effect.Effect<ReadonlyArray<ReleaseCandidate>, IndexerError>
}
...
function buildUrl(baseUrl: string, apiKey: string, params: Record<string, string | number | undefined>): URL {
...
function checkTorznabError(parsed: unknown, config: IndexerConfig): Effect.Effect<void, IndexerError> {
...
function parseReleases(parsed: unknown, config: IndexerConfig): ReadonlyArray<ReleaseCandidate> {
  ...
  return items.map((item: Record<string, unknown>): ReleaseCandidate => {
    ...
    return {
      ...
      seeders: protocol === 'torrent' ? Number(getAttr(item as Record<string, unknown>, 'seeders') ?? 0) : null,
      leechers: protocol === 'torrent' ? Number(getAttr(item as Record<string, unknown>, 'peers') ?? 0) : null,
      age: ageDays,
      downloadUrl: String(item.link ?? ''),
      infoUrl: item.comments ? String(item.comments) : item.guid ? String((item.guid as Record<string, unknown>)?.['#text'] ?? item.guid) : null,
