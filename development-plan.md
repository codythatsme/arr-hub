...
- SQLite/Drizzle schema for users, API keys, movies, series/seasons/episodes, quality profiles, custom formats, tags, indexers, download clients, media servers, queue, notifications, plugins, release decisions, scheduler, onboarding, and Plex playback history.
...
- The Radarr/Sonarr/Prowlarr REST API compatibility layer has only started: movies, series, episodes, indexers, download clients, tags, root folders, quality profiles, custom formats, commands, queue, history, wanted, calendar, system status, and health now have first-pass compatible endpoints and public OpenAPI documentation, but the broader API surface is still missing.
...
- `bun run test`: passed outside the sandbox for the SMTP bind test, 72 test files plus 1 skipped live suite, 612 passed and 4 skipped tests.
...
- Focused auth/startup tests passed for persistent login lockout, failed-attempt cleanup after successful login, TRPC 429 mapping, startup schema validation, and environment-gated password recovery.
...
- `03efbbae43` added first-pass compatible `/api/v1|v3/rootfolder` routes with list/get/create/delete handlers and Arr-style disk-space/accessibility resource mapping.
- `87405afb57` added first-pass compatible `/api/v1|v3/customformat` routes with list/get/create/update/delete handlers and Arr-style specification resource mapping.
- `a3c592b987` added first-pass compatible `/api/v1|v3/calendar` routes with date-range and episode-id handlers backed by Sonarr-style episode calendar resources.
- `155983ca83` added first-pass compatible `/api/v1|v3/queue`, `/api/v1|v3/queue/details`, `/api/v1|v3/queue/status`, and queue item delete routes backed by local queue rows and Arr-style queue resources.
- `21d45b8624` added first-pass compatible `/api/v1|v3/history`, `/api/v1|v3/history/since`, `/api/v1|v3/history/movie`, and `/api/v1|v3/history/series` read routes backed by operational history rows and Arr-style history resources.
...
- `550acd59c8` added first-pass compatible `/api/v1|v3/episode` list/read/update-monitor routes with Sonarr-style episode resource mapping and series/season/file filters.
- `c8b47fb05e` added first-pass compatible `/api/v1|v3/indexer` list/create and `/api/v1|v3/indexer/{id}` read/update/delete routes with Arr-style provider resources, masked API-key fields, health/capability mapping, and Torznab/Newznab/Cardigann input parsing.
- `ab1cb05f0b` added first-pass compatible `/api/v1|v3/downloadclient` list/create and `/api/v1|v3/downloadclient/{id}` read/update/delete routes with Arr-style provider resources, masked password fields, protocol/category mapping, and partial settings-safe input parsing.
...
- `9280b6383b` added public OpenAPI documentation endpoints for the implemented REST API surface.
...
- `src/effect/services/NotificationService.ts` and `SmtpClient.ts`: in-app, generic webhook, Discord, Slack, Ntfy, Gotify, Telegram, Pushover, Apprise, Notifiarr, custom script, and SMTP email notification channels.
...
- `TitleParserService` handles common movie/TV patterns, season packs, proper/repack/version revision metadata, and first-pass anime absolute episode numbering, with a vendor-inspired fixture corpus.
...
- Operators can temporarily set `ARR_HUB_PASSWORD_RECOVERY_TOKEN` to recover the admin password from the login screen; successful recovery revokes active sessions and API keys for that admin.
- API keys now support full-app, REST read-only, and REST read/write scopes. tRPC app access requires sessions or full-app API keys; compatible HTTP routes require REST read scope for GET/HEAD/OPTIONS and REST write scope for mutating methods; aggregate Torznab/Newznab feeds require REST read scope.
...
- The Sonarr/Radarr/Prowlarr-compatible REST API has first-pass movie, series, episode, indexer, download client, tag, root folder, quality profile, custom format, calendar, queue, history, command, wanted, system status, health, and OpenAPI coverage. `/api/v1|v3/movie`, `/api/v1|v3/series`, `/api/v1|v3/episode`, `/api/v1|v3/indexer`, `/api/v1|v3/downloadclient`, `/api/v1|v3/tag`, `/api/v1|v3/tag/detail`, `/api/v1|v3/rootfolder`, `/api/v1|v3/qualityprofile`, `/api/v1|v3/customformat`, `/api/v1|v3/calendar`, `/api/v1|v3/queue`, `/api/v1|v3/history`, `/api/v1|v3/command`, `/api/v1|v3/wanted/missing`, `/api/v1|v3/wanted/cutoff`, `/api/v1|v3/system/status`, and `/api/v1|v3/health` accept Bearer, `X-Api-Key`, or `apikey` authentication with API-key scope enforcement and expose movie CRUD, series CRUD, episode reads/monitor updates, indexer CRUD, download client CRUD, tag CRUD/detail, root folder CRUD, quality profile CRUD, custom format CRUD, calendar reads, queue reads/status/delete, history reads, scheduler-backed command list/create/read/delete, wanted missing/cutoff reads, system status, and health responses. `/api/openapi` and `/api/v1|v3/openapi` expose unauthenticated OpenAPI JSON for the implemented public REST surface.
...
- [x] Add password reset/recovery flow. Current coverage is operator-controlled via temporary `ARR_HUB_PASSWORD_RECOVERY_TOKEN`, login-screen reset UI, and session/API-key revocation on success.
- [x] Add API key scoping if external API compatibility is implemented. Current coverage stores scopes on API keys, defaults existing/new keys to full-app access for compatibility, exposes scope selection/display in Security settings, restricts tRPC app procedures to sessions/full-app keys, and enforces REST read/write scopes on compatible HTTP routes and aggregate feeds.
- [x] Decide whether to implement compatible `/api/v3` Sonarr/Radarr-style endpoints and Prowlarr-style `/api/v1`/Torznab endpoints. Current decision is incremental compatibility, starting with movies, series, episodes, indexers, download clients, tags, root folders, quality profiles, custom formats, calendar, queue, history, commands, wanted, system status, health, and the existing aggregate Torznab/Newznab endpoints.
...
- [x] Add first compatible episode read/monitor endpoints for `/api/v1|v3/episode`.
- [x] Add first compatible indexer CRUD endpoints for `/api/v1|v3/indexer`.
- [x] Add first compatible download client CRUD endpoints for `/api/v1|v3/downloadclient`.
...
- [x] Add first compatible custom format REST endpoints for `/api/v1|v3/customformat`.
- [x] Add first compatible calendar read endpoints for `/api/v1|v3/calendar`.
- [x] Add first compatible queue read/status/delete endpoints for `/api/v1|v3/queue`.
- [x] Add first compatible history read endpoints for `/api/v1|v3/history`.
- [x] Add first compatible command list/create/read/delete endpoints for `/api/v1|v3/command`.
- [x] Add first compatible wanted missing/cutoff read endpoints for `/api/v1|v3/wanted`.
- [x] Add first compatible system status and health endpoints for `/api/v1|v3/system/status` and `/api/v1|v3/health`.
- [x] Add OpenAPI or equivalent docs for public APIs. Current coverage exposes OpenAPI JSON at `/api/openapi` and `/api/v1|v3/openapi`.
...
- In-app, generic webhook, Discord, Slack, Ntfy, Gotify, Telegram, Pushover, Apprise, Notifiarr, custom script, and SMTP email channels exist.
...
- [x] Add first-pass Discord, Slack, Ntfy, Gotify, Telegram, Pushover, Apprise, Notifiarr, custom script, and email/SMTP provider adapters using provider-specific webhook/topic/message/API/passthrough/env/SMTP payloads and Settings channel selection.
...
- Tags still do not drive release restrictions, client selection, notification routing, import lists, release profiles, delay profiles, custom filters, or automation.
...
- First-pass authenticated episode read/monitor endpoints exist for `/api/v1|v3/episode`.
- First-pass authenticated indexer CRUD endpoints exist for `/api/v1|v3/indexer`.
- First-pass authenticated download client CRUD endpoints exist for `/api/v1|v3/downloadclient`.
...
- First-pass authenticated calendar read endpoints exist for `/api/v1|v3/calendar`.
- First-pass authenticated queue read/status/delete endpoints exist for `/api/v1|v3/queue`.
- First-pass authenticated history read endpoints exist for `/api/v1|v3/history`.
- First-pass authenticated command list/create/read/delete endpoints exist for `/api/v1|v3/command`.
- First-pass authenticated wanted missing/cutoff read endpoints exist for `/api/v1|v3/wanted`.
- First-pass authenticated system status and health endpoints exist for `/api/v1|v3/system/status` and `/api/v1|v3/health`.
- Public OpenAPI JSON exists for the implemented REST surface at `/api/openapi` and `/api/v1|v3/openapi`.
...
- Compatible read/write endpoints for movies, series, episodes, queue, history, wanted, calendar, commands, indexers, download clients, root folders, quality profiles, custom formats, tags, system status, and health. Current REST compatibility coverage is movies, series, episodes, indexers, download clients, tags, root folders, quality profiles, custom formats, commands, queue, history, wanted, calendar, system status, and health only.
