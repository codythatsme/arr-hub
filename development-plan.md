...
- The Radarr/Sonarr/Prowlarr REST API compatibility layer has only started: tags, root folders, quality profiles, custom formats, commands, queue, history, calendar, system status, and health now have first-pass compatible endpoints, but the broader API surface is still missing.
...
- Focused compatible API tests passed for system status, health, root folder, quality profile, custom format, command, queue, history, and calendar resource mapping.
...
- `src/integrations/http/compatTags.ts`, `src/integrations/http/compatSystem.ts`, `src/integrations/http/compatQualityProfiles.ts`, `src/integrations/http/compatRootFolders.ts`, `src/integrations/http/compatCustomFormats.ts`, `src/integrations/http/compatCalendar.ts`, `src/integrations/http/compatQueue.ts`, `src/integrations/http/compatHistory.ts`, and `src/integrations/http/compatCommands.ts`: first-pass authenticated Arr-compatible tag, root folder, quality profile, custom format, calendar, queue, history, command, system status, and health REST routes for `/api/v1` and `/api/v3`.
...
- [x] Decide whether to implement compatible `/api/v3` Sonarr/Radarr-style endpoints and Prowlarr-style `/api/v1`/Torznab endpoints. Current decision is incremental compatibility, starting with tags, root folders, quality profiles, custom formats, calendar, queue, history, commands, system status, health, and the existing aggregate Torznab/Newznab endpoints.
...
- Compatible read/write endpoints for movies, series, episodes, queue, history, wanted, calendar, commands, indexers, download clients, root folders, quality profiles, custom formats, tags, system status, and health. Current REST compatibility coverage is tags, root folders, quality profiles, custom formats, commands, queue, history, calendar, system status, and health only.
