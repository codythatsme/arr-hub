...
- Prowlarr replacement now has a first-pass foundation. The app consumes Torznab/Newznab endpoints and exposes authenticated aggregate Torznab/Newznab feeds plus persisted generic definitions, small Cardigann-style YAML fixture definitions including Nyaa RSS coverage with first-pass GET/POST XML request execution and templated request headers, version-aware built-in definition refresh, URL-backed Cardigann definition source refresh with DB-backed runtime loading, scheduler execution, and checksum provenance/pinning, proxy records applied to outbound requests, search stats, first-pass health/backoff handling, per-indexer minimum-seeder, query-cooldown, and rolling query/grab-limit policy controls, and persisted Radarr/Sonarr aggregate app sync with scheduler execution, indexer add/update/remove triggers, stale remote aggregate cleanup, and app-specific torrent seed criteria. It still does not manage a Prowlarr-scale tracker catalogue, full Cardigann selector/login parity, full app-sync parity for richer per-indexer and app-specific sync semantics, deeper policy parity beyond first-pass search/grab limits, or a trusted first-party remote definition catalogue distribution pipeline.
...
- `bun run lint`: passed with 297 warnings and 0 errors.
...
- `src/effect/services/IndexerService.ts`, `src/effect/services/CardigannDefinitionLoader.ts`, `src/effect/services/CardigannAdapter.ts`, `src/effect/services/TorznabAdapter.ts`, `src/effect/services/IndexerDefinitionSourceService.ts`, and `src/effect/services/IndexerApplicationService.ts`: Torznab/Newznab connection testing and search, generic and curated Cardigann-style first-party definition seeding including Nyaa RSS coverage, version-aware built-in definition refresh, URL-backed Cardigann definition source refresh with stored raw YAML, SHA-256 provenance/pinning, and enabled-source scheduler refresh, first-pass Cardigann GET/POST XML search execution with templated request headers, proxy configuration persistence and outbound application, search stats, first-pass search health/backoff, per-indexer minimum-seeder, query-cooldown, and rolling query/grab-limit policy controls, aggregate protocol capability support, and persisted Radarr/Sonarr aggregate app sync with enabled-app scheduler refresh, indexer mutation triggers, stale remote aggregate cleanup, and app-specific torrent seed criteria.
