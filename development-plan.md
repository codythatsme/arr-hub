...
- `bun run test`: passed, 40 test files plus 1 skipped live suite, 443 passed and 4 skipped tests.
...
- There is no broad first-party tracker catalogue, full Cardigann selector/login runtime beyond first-pass HTML selector/case/pseudo-filter/positional-filter/selector-list/template-rendering/row-filter/date-header/field-filter features, JSON response selector parsing with row count/attribute metadata, parent selectors, selector positional filters, and selector pseudo-filters, XML response selector parsing, magnet result-field aliasing, categorydesc result-field mapping, zero volume-factor preservation, field-name modifier handling, optional field fallback preservation, search preprocessing filters, response no-results message handling, and cookie/one-url/form login execution with multipart forms, selector-derived form values, configured CAPTCHA responses, simpleCaptcha forms, manual CAPTCHA auth fields, informational auth notes, and login test selectors, richer indexer-specific auth UX beyond first-pass cookie/2FA notes, full rate-limit policy engine beyond query cooldowns and rolling query/grab limits, full trusted first-party remote definition catalogue distribution pipeline, or full Prowlarr app-sync parity for richer per-indexer and app-specific sync semantics.
