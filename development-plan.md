...
- The Radarr/Sonarr/Prowlarr REST API compatibility layer has only started: movies, series, episodes, indexers, download clients, tags, root folders, quality profiles, custom formats, commands, queue, history, wanted, calendar, system status, and health now have first-pass compatible endpoints, but the broader API surface is still missing.
...
- Focused compatible API tests passed for system status, health, root folder, quality profile, custom format, movie, series, episode, indexer, download client, command, queue, history, wanted, and calendar resource mapping/input parsing.
