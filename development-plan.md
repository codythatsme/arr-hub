...
- `bun run test`: passed, 40 test files plus 1 skipped live suite, 475 passed and 4 skipped tests.
...
- Prowlarr has 143 files under `Indexers/Definitions` and 16 first-level definition families. ARR Hub has generic Torznab/Newznab consumption, a small Cardigann-style built-in set including Nyaa RSS, Anidex HTML selector, TorrentsCSV JSON selector, SubsPlease JSON selector, Knaben JSON POST body, AnimeTosho, MoreThanTV, HDAccess, and Torrent Network coverage, manually configured URL-backed Cardigann source refresh with scheduler execution, and aggregate app sync, but not broad tracker coverage.
