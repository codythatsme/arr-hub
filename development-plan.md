...
- `bun run test`: passed, 40 test files plus 1 skipped live suite, 496 passed and 4 skipped tests.
...
- Prowlarr has 143 files under `Indexers/Definitions` and 16 first-level definition families. ARR Hub has generic Torznab/Newznab consumption, a small Cardigann-style built-in set including Nyaa RSS, Anidex HTML selector, TorrentsCSV JSON selector, SubsPlease JSON selector, Knaben JSON POST body, TorrentDay cookie-auth JSON selector, IPTorrents cookie-auth HTML selector, RetroFlix and SpeedApp.io JSON selectors, BeyondHD JSON POST selector, BitHDTV cookie-auth HTML selector, TorrentBytes POST-login HTML selector, SceneHD passkey JSON selector, SceneTime cookie-auth HTML selector, HD-Space form-login HTML selector, SpeedCD cookie-auth HTML selector, HD-Torrents POST-login HTML selector, FunFile POST-login HTML selector, ImmortalSeed POST-login HTML selector, XSpeeds POST-login HTML selector, RevolutionTT POST-login HTML selector, PreToMe form-login HTML selector, AnimeTorrents cookie-auth AJAX HTML selector, BakaBT form-login HTML selector, Nebulance JSON-RPC API selector, ShizaProject GraphQL JSON selector, AnimeTosho, MoreThanTV, HDAccess, and Torrent Network coverage, manually configured URL-backed Cardigann source refresh with scheduler execution, and aggregate app sync, but not broad tracker coverage.
