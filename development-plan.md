...
- Focused required live common-indexer gate failed fast with the exact missing credential variable names when run without live credentials.
...
- `18551469a0` documented the unavailable real-provider common-indexer run as an external release gate and completed the local Milestone 5 plan without claiming upstream-specific interoperability.
...
- Settings includes a tag library that can create tags, show first-pass usage counts, and delete unused tags. Settings also exposes saved auto-tagging rules, manual rule reapply, and saved custom filters.
